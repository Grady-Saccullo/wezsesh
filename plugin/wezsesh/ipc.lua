-- ipc.lua — `user-var-changed` handler. Owns ONE wezterm event hook
-- (USER_VAR_NAME) and walks every inbound request through the same
-- linear gate: parse pointer → read sidecar request file → match the
-- emitting pane to a known spawn → verify HMAC → check freshness /
-- replay / target-window → dispatch.
--
-- Why a sidecar instead of inlining the request in the OSC: wezterm's
-- OSC payload ceiling is 256 B and arbitrary user-var values are
-- attacker-controlled until HMAC-verified. The OSC carries a fixed-
-- width pointer (id + path); the path lives under <runtime_dir>/req/
-- and is mode-0600 owner-only. Pre-steps (1)–(4) reject any pointer
-- whose path escapes the prefix or whose stat fails the regular-file /
-- non-symlink / owner-self / mode-bits guard.
--
-- Why the handler is strictly synchronous up to the dispatch boundary:
-- `user-var-changed` fires on the wezterm event loop. Any
-- `run_child_process` / `sleep_ms` / async wezterm primitive in the
-- gate would block the renderer. Step (i)'s
-- `wezterm.background_child_process` (inside ops.dispatch via
-- result.lua) is fire-and-forget and exempt.
--
-- Why everything is `pcall`-wrapped at the boundary: an uncaught raise
-- in any handler registered with `wezterm.on` wedges the wezterm event
-- loop. The outer `M.register` and every JSON / canonical / HMAC step
-- inside `handle_user_var` is therefore `pcall`-protected; failures
-- log via runtime.log and silently drop on the wire so a malformed
-- pointer can't be used as a denial-of-service signal.
--
-- mlua sandbox: every external require is namespaced (`wezsesh.*`) so
-- init.lua's cache-bust loop on Ctrl+Shift+R picks up edits. The
-- standalone spec (`ipc_spec.lua`) installs harness doubles via
-- `package.preload[...]` BEFORE requiring this file.
...
-- the validator below budgets `len(runtime_dir) + 14 ≤ ceiling`.
local SUN_PATH_TAIL_BYTES  = 14   -- "/<8hex>.sock"

-- Pluggable seam for the request-pointer stat() check. Production
...
-- Internal deps table. Two genuine production seams left after log
-- routing moved to runtime.log: `now` (clock — for freshness window
-- determinism in tests) and `stat_path` (the file-mode guard above).
-- `dispatch` is lazy-resolved at handler-call time so this module can
-- load in a session where ops.lua is not yet available.
M._deps = {
    now       = os.time,
...
-- a stat double, a clock, and a dispatch double. Production code
-- never calls this.
...
-- ────────────────────────────────────────────────────────────────────
-- pointer field-shape validator (pre-step 2)
-- ────────────────────────────────────────────────────────────────────
--
...
-- ────────────────────────────────────────────────────────────────────
-- payload field-shape validator (step (d))
-- ────────────────────────────────────────────────────────────────────
--
...
--
-- HOME expansion: `~/` prefix is replaced with `wezterm.home_dir`
-- (preferred) or `$HOME` (fallback). On expansion failure the original
-- string is used; the SUN_PATH math then has the same effect.
...
local function platform_sun_path_ceiling()
    -- darwin = 104; Linux/everything else = 108. Matching on
    -- `-apple-darwin` (rather than the leading triple component) so a
    -- future BSD addition still picks the safe (108) ceiling.
...
-- ────────────────────────────────────────────────────────────────────
--
-- Returns true iff the file at `path` passes the request-pointer
-- guards:
...
--
-- If `_deps.stat_path` returns nil (no stat available — production on
-- a wezterm without an `lfs` binding), the path-prefix check from
-- pre-step (2) plus the parent dir's `safefs.Enforce(SymlinkRefuse)`
-- guarantee on the binary side carry the safety burden. The function
...
local function read_request_file(path)
    ...
    fh:close()
    -- Best-effort unlink; orphans are reaped by the binary's
    -- startup-sweep on the next TUI launch.
    os.remove(path)
    return bytes
end
...
function M.handle_user_var(window, pane, name, value, opts)
    ...
    -- The pane that emitted the OSC is the binary spawn this handler is
    -- responsible for; `manager.spawn` recorded the {target_window_id,
    ...
    local hex_key = globals.session_key()
    if type(hex_key) ~= "string" or #hex_key ~= 64 then
        -- Session-key generation failed at apply_to_config; the toast
        -- was already shown there. Drop silently.
        return
    end
    ...
    -- The verifier MUST drop the `hmac` field, tag the remaining tree
    -- by the verb-keyed shape, and canonical-encode the result.
    -- Encoder raises on non-canonical input (untagged tables, floats,
    -- invalid UTF-8); pcall keeps the raise from wedging the event
    -- loop and we silently drop on the wire while logging for
    -- observability.
    ...
    -- key (it IS part of the on-the-wire envelope), so the verifier
    ...
    -- Wire `-1` is the "any window" sentinel — skip the window-match
    -- check (the per-pane `state.set_state` record is the gating
    -- signal in that branch). Any other value is a real wezterm window
    -- id and is matched STRICTLY against the live pane's recorded
    -- `session.target_window_id`; `0` is wezterm's first-window id and
    -- matches `session.target_window_id == 0` (no sentinel collision
    -- because the sentinel is `-1`).
    ...
    -- session entry: GLOBAL reads return deserialised snapshots, so
    -- explicit set_state write-back is required after any sub-table
    -- mutation. `spawned_at` is intentionally NOT re-stamped — its
    -- value gates the binary's startup-sweep age window.
