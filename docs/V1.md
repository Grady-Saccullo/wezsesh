# V1 — wezsesh as a work-intelligence layer

> **Status:** strategic vision document. Picked up *after* the current
> `PROJECT.md` ledger lands v0.1 (Phase 0–9 complete, Phase 10 integration
...
> file is the **bridge** — what to build, why, and what order.
...
interesting questions — the ones that matter to a developer who lives in
the terminal all day — are larger:
...
A workspace switcher answers none of these. A **work journal** — a
time-aware record of your terminal activity — answers all of them.
...
- There's no record of what happened — every session begins amnesic.
...
## Foundation — what v1 keeps from v0.1
...
| `new` | Create a new workspace (cwd-rooted) | Records `create` event. Optional path through a template (Phase 15 — `T-1403`). Existing flow unchanged. |
...
| `pin` | Toggle pin (live OR saved, single source of truth via §13.11) (§13.12.4) | Pinned workspaces always at the top of the picker; pin slots 1–9 → numeric quick-jump keybindings registered alongside `<leader>+W`. Existing storage rules unchanged. |

### TUI affordances (preserved, enriched)

- **`/` filter** — fuzzy filter across visible rows. v1 adds match
  count in the input line and per-row match highlighting.
- **Sort modes** — `live_first` (current default), `alpha`, `recent`,
...
- `wezsesh trust [--rebind / --revoke / --list / --prune / --show / --path / --sidecar]`
  — trust management; v1 surfaces these inside the TUI's Trust tab
  too. CLI surface preserved.
- `wezsesh reset` (and `nuke` alias) — wezsesh-managed dir cleanup;
  unchanged.
- `wezsesh keygen` — HMAC key generation; unchanged.
- `wezsesh --version` — version stamp; unchanged.
...
length-prefixed trust-hash construction — **all preserved exactly,
...
The IPC wire path — canonical-JSON byte-equality (§4.1, §17.1),
HMAC field-removal sequence (§4.3), OSC ≤ 256 B forward path
(spike #3), Unix-socket reverse path (§3.4), the §3.3 request
envelope shape, the §6 verb-keyed shape table — **stable across
...
single name yet — the **layout did the work**.
...
**You don't need to switch yet — you already know what you were doing.**
This preview is a complete context recovery in one screen.

### 3:00pm — "Slack ping. Drop everything."
...
free — wezsesh saw every switch, every save, every hook fire.

### Day 14 — "Where was that test that was failing?"
...
This is **time travel for terminal work** — something no other tool can
...
`{paneID, cwd, foreground_process_name, foreground_process_args}` —
...
│                                          │ │ ─────────────────────────────────── │
│ ── Older ──────────────────────────────  │ │ Notes (TODO):                       │
│    legacy      master          —     31d │ │ - normalize Bearer header at parse  │
│                                          │ │   level, then re-test               │
│                                          │ │ ─────────────────────────────────── │
│ 📊 ▁▂▅█▇▅▂▁▁▁▂▃ activity · 30d           │ │ Trust: ✓ on_create  ✓ on_restore    │
╰──────────────────────────────────────────╯ ╰─────────────────────────────────────╯
╭─ Status ────────────────────────────────────────────────────────────────────────╮
│ today: 3h api-server · 45m docs · 1h customer-bug   |   ✓ trust  ✓ resurrect     │
╰─────────────────────────────────────────────────────────────────────────────────╯
...
|---|---|---|
| **1. Workspaces** | the screen above — picker, frecency-tiered, rich preview | The default. 90% of usage. |
...
| **3. Journal** | the 6pm timeline view — chronological event stream with grouping | Standup. Time-tracking. "What did I do?" |
...
| `bubbles.list` (custom delegate) | The picker — replaces hand-rolled `visibleRows`/`cursor` |
...
These features aren't a list — they **compound**:
...
list — no events, no time, no context.
...
Four phases. Not Phase 10 / 11 — those are v0.1 finishing work. These
are v1 phases.

### Phase 12 — work-intelligence foundation (≈ 2 weeks)
...
### Phase 13 — TUI re-architecture (≈ 2 weeks)
...
| T-1204 | Tabbed views — Workspaces, Activity, Journal, Trust, Doctor |

### Phase 14 — the killers (≈ 3 weeks)
...
**Total:** roughly 6–8 weeks of focused implementation alone.
...
ceiling, the Unix-socket reverse path — all stable. v1 adds new verbs
...
1. **It's the only direction where wezsesh has a moat.** Frecency sort,
   colored markers, fancier panels — every other session manager will
...
2. **The data is already free.** Every architectural piece — switch
   counts, snapshot states, hook outcomes, pane scrollback — already
