...
#### Save serialization via flock (added v2.0)

The Lua-side `save` handler uses POSIX advisory locks via `fcntl(F_SETLK, F_WRLCK)` (or the `flock(2)` syscall on systems where it is preferred and equivalent) on the snapshot file path. Lua does not expose `fcntl` natively, so the lock is acquired and released by the Go binary through a dedicated IPC sub-step:

1. Binary's `save` handler invokes (locally, before emitting OSC) `safefs.AcquireExclusive(snapshotPath, 5*time.Second)`. This call:
   - Opens the file with `O_RDWR|O_NOFOLLOW`.
   - Calls `fcntl(F_SETLKW)` with a 5-second deadline (via `unix.FcntlFlock` + a watchdog goroutine that signals SIGALRM-equivalent on overrun, OR a non-blocking `F_SETLK` retried with backoff).
   - Returns the file descriptor + a `release()` closure.
   - On timeout: returns `SNAPSHOT_LOCKED`.
...
3. If hash matches → emit `save` OSC to Lua. Lua calls `resurrect.state_manager.save_state(...)`. The lock is held throughout (file descriptor passed through GLOBAL keyed by request ID for the Lua side to honor; OR — simpler — the binary holds the lock on its own fd while resurrect writes via its own fd, since POSIX locks are per-process not per-fd, the binary's lock blocks any other binary's lock-acquire attempt).
4. After resurrect's write completes (signaled by `file_io.write_state.finished`), the binary releases the lock.

**Cross-binary semantics**: POSIX advisory locks are per-process; a second wezsesh binary attempting `F_SETLK` on the same path while the first holds it gets `EAGAIN`/`EWOULDBLOCK`. The 5s deadline bounds wait time. NFS `fcntl` locks are weakly defined; we document the limitation and recommend users on NFS-mounted snapshot dirs serialize externally.
...
// AcquireExclusive opens path with O_RDWR|O_NOFOLLOW and acquires a POSIX
// advisory lock (fcntl F_SETLKW) with a deadline. Returns the fd and a
// release closure. Used by §6.7 save / sidecar write serialization.
// On timeout: returns ErrLockTimeout.
func AcquireExclusive(path string, deadline time.Duration) (fd int, release func(), err error)
```
...
**Status**: design v2.0 (round-9 bubbletea-v2 / payload-budget / TOCTOU / Lua-payload-robustness audit complete).
