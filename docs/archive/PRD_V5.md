...
1. Before emitting the OSC, the binary creates `<reply_dir>/<8-hex>.sock`:
   ...
   - **SUN_PATH validation at config-load (added v2.2)**: the Lua-side `apply_to_config` MUST validate `len(opts.runtime_dir) + 14 ≤ ceiling` (104 on darwin, 108 on Linux) and raise a config error with the exact remediation message. Without this check, a user who sets `opts.runtime_dir` to a long path (e.g., `~/Library/Application Support/wezterm/wezsesh/sockets/`) silently passes config load, then every TUI launch fails at `net.Listen("unix", path)` with `ENAMETOOLONG`. The TUI surfaces a generic `IPC_TIMEOUT` and the root cause is invisible. Validation example (Lua):
     ```lua
     local ceiling = (wezterm.target_triple:find("apple") and 104) or 108
     local needed = #opts.runtime_dir + 14   -- "/<8hex>.sock"
     if needed > ceiling then
         error(string.format(
             "wezsesh: runtime_dir too long for AF_UNIX SUN_PATH " ..
             "(needed=%d, ceiling=%d). Shorten the path or use the default.",
             needed, ceiling))
     end
     ```
     The Go binary independently re-validates at runtime as defense-in-depth (returns `IPC_INIT_FAILED` with the same diagnostic before even attempting `net.Listen`).
...
Layer 2 matches the resolved (NFC-normalized, symlinks-followed-once-via-`filepath.EvalSymlinks`) absolute path against these prefixes:
...
- `~/iCloud Drive` (legacy symlinked alias, still common)
- `~/Dropbox` (legacy direct-mount layout, pre-File-Provider)
- `~/Google Drive` and `/Volumes/GoogleDrive*` (legacy macFUSE Google Drive)
- `~/Desktop` and `~/Documents` *only* if iCloud Drive's "Desktop & Documents" sync is enabled (detect via the presence of `~/Library/Mobile Documents/com~apple~CloudDocs/Desktop` or `Documents` symlink; if found, the `~/Desktop` and `~/Documents` paths under `$HOME` are also iCloud-backed).
...
**Known false-negative class** (documented honestly; v2.2). A snapshot dir placed at a cloud-sync path NOT on Layer 2's prefix list (e.g., a custom symlink under a non-default location, or a third-party File Provider extension we don't recognize) will silently bypass detection. The WARN is best-effort; the actual data-loss defense is the recommendation that snapshot dirs live under `~/.local/share/wezterm/state/resurrect/` (the resurrect default; explicitly NOT in any cloud-sync directory).
...
//
// CRITICAL fd-hygiene contract (added v2.2): POSIX advisory-lock semantics
// release ALL locks the process holds on a file when the process closes
// ANY fd referring to that file (verified fcntl(2) Linux + macOS man pages;
// "If a process closes any file descriptor referring to a file, then all of
// the process's locks on that file are released, regardless of the file
// descriptor(s) on which the locks were obtained"). This means: while
// AcquireExclusive's fd is live, NO other code in the same process may
// open() the same path/inode. Doing so and then closing that second fd
// silently drops the lock without notification. Enforcement:
...
//   3. On Linux, AcquireExclusive uses F_OFD_SETLK (Open File Description
//      locks) when available — these are bound to the fd, not the process,
//      and survive close-of-other-fd. F_OFD_SETLK is Linux-only since 3.15.
//      darwin / FreeBSD lack OFD locks; the multi-fd discipline above is
//      mandatory on those platforms (verified absent in current macOS SDK).
//   4. CI test: opens a path, takes the lock, opens-and-closes a second fd
//      to the same path, attempts a second AcquireExclusive from a
//      subprocess — must succeed on darwin (lock dropped, demonstrating
//      the footgun) and FAIL on Linux (OFD lock survived).
...
// goroutine — Statfs itself can block on a hung NFS server, so this helper
// designed to warn about hang surfaces would otherwise become one. On
// Statfs timeout, returns ("unknown", true, nil) — treats unprovable paths
// as network to err on the side of cautionary WARN.
//
// Used by §6.7 startup WARN and §7.3 doctor. Documented false-negative
// class: cloud-sync paths NOT on the Layer 2 prefix list (e.g., custom
// symlink layouts, third-party File Provider extensions) silently bypass
// detection.
...
**Status**: design v2.2 (round-11 v2.1-edits-revalidation / File-Provider-cloud-sync / resurrect-API-runtime-validation / encryption-hang-surface audit complete).
