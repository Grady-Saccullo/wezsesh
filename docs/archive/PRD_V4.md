...
**Network filesystem detection** (added v2.1): before opening the snapshot dir, the binary calls `safefs.IsNetworkFS(snapshotPath)` (uses `unix.Statfs` + `f_type` check on Linux; `unix.Statfs` + `f_fstypename` on darwin matching against `nfs`, `smbfs`, `webdav`, `osxfuse`, `afpfs`, `autofs`). Behavior on detect:
...
- **iCloud Drive / Dropbox / Google Drive (osxfuse)**: cloud-sync agents perform atomic-replace (delete + create new inode) WITHOUT honoring advisory locks (verified [eclecticlight on APFS sync semantics](https://eclecticlight.co/2023/11/15/backup-errors-icloud-drive-and-the-limits-of-apfs/)). Held locks are inode-bound and become orphaned on sync rewrite. The binary surfaces WARN: `wezsesh: snapshot dir is cloud-synced — sync rewrites bypass file locks. Concurrent saves may silently lose data. Recommend a non-synced path (e.g., ~/.local/share/wezterm/state/resurrect/).` Operation is not refused; the user accepted this risk by choosing the path.
- **autofs / SMB / SSHFS**: same WARN as NFS.
- `wezsesh doctor` reports the snapshot dir's FS type and any of the above conditions.
...
// non-local filesystem where POSIX advisory locks may behave incorrectly:
// NFS (lockd-dependent, kernel hang bugs), SMB/CIFS, WebDAV, SSHFS/osxfuse,
// AFP, autofs, plus darwin cloud-sync providers (iCloud Drive, Dropbox,
// Google Drive — all FUSE-mounted under osxfuse with sync agents that do
// not honor advisory locks). Implementation: unix.Statfs + f_type (Linux)
// or f_fstypename (darwin). Used by §6.7 startup WARN and §7.3 doctor.
func IsNetworkFS(path string) (fsType string, isNetwork bool, err error)
```
...
**Status**: design v2.1 (round-10 fcntl-semantics / API-revalidation / GLOBAL-concurrency / project-sidecar-trust audit complete).
