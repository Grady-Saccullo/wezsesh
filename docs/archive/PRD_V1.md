...
**Canonical JSON for HMAC** (must be byte-identical on both sides):
...
3. Lua handler dispatches the op, then emits the reply via:
   ```lua
...
│       ├── canonical_json.lua ← canonical-JSON serializer (matches Go side)
│       ├── hmac.lua          ← HMAC-SHA-256 wrapper (RFC 2104; ~30 LOC)
...
The `hmac` field in every payload is computed over **canonical JSON** (keys sorted lex, no whitespace, integers as decimal, UTF-8, hmac field omitted). Both Go and Lua sides must agree byte-for-byte.
...
| **Binary unexpected exit / panic** (pane-closed event fires for the wezsesh pane while a request is in `wezterm.GLOBAL.wezsesh_requests`) | Immediately surface `UNEXPECTED_EXIT` toast in the parent wezterm window; do NOT wait for `IPC_TIMEOUT`. Subscribe to `wezterm.on('pane-closed', ...)` and check `pane_id` against in-flight `wezsesh_state[pid_key]` entries. |
...
- Golden tests for canonical-JSON byte-equality between Go (`internal/canonicaljson/`) and Lua (`plugin/wezsesh/canonical_json.lua`). Test vectors include unicode, integer edge cases (0, -1, 2^53), nested structures.
- HMAC compatibility: Lua signs a fixture, Go verifies; Go signs a fixture, Lua verifies. Run on every CI build.
...
**Status**: design v1.8 (round-7 secondary-defense / supply-chain / lifecycle audit complete), ready to start implementation.
